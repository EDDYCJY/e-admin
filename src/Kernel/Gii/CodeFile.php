...
class CodeFile extends Object
{
    ...
    public function save()
    {
        ...
        if ($this->operation === self::OP_CREATE) {
            ...
            if (!is_dir($dir)) {
                $mask = @umask(0);
                $result = @mkdir($dir, isset($module->newDirMode) ? $module->newDirMode : 0777, true);
                @umask($mask);
                if (!$result) {
        ...
        if (@file_put_contents($this->path, $this->content) === false) {
            ...
            $mask = @umask(0);
            @chmod($this->path, isset($module->newFileMode) ? $module->newFileMode : 0777);
            @umask($mask);
        }
